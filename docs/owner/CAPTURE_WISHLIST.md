...
| 2 | **MVP Baseball 2005** `SLUS-21135` | a **replay or cutscene camera close-up on a player**, or a portrait at full size | GS dump | The `0x0E` codec. The per-draw dumps proved the three existing frames draw the **low LOD** and never request the `0x0E` parts of a bank at all, so a close-up that forces the high-detail LOD is the capture that matters. Unblocks **23,954 images** |
